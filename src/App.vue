            ...
            <div class="column col-auto fullh left-side">
                    ...
                    <ul class="menu" v-if="tab == 0">
                        <li class="menu-item" v-for="(f,i) in favlist" :key="f.time">
                            <a class="c-hand" @click="req.history = f;refresh=Date.now()">
                                <span class="label mr-1 tooltip tooltip-right" :data-tooltip="f.type" :class="{'label-success':f.type == 'get',
                ...
                'label-warning':f.type == 'delete'
                    ...
                    <ul class="menu" v-else>
                        ...
                        <li class="menu-item tooltip tooltip-bottom" v-for="(h,i) in history" :key="h.time" :data-tooltip="new Date(h.time).format('yyyy-MM-dd HH:mm:ss')">
                            <a class="c-hand" @click="req.history = h;refresh=Date.now()">
                                <span class="label mr-1 tooltip tooltip-right" :data-tooltip="h.type" :class="{'label-success':h.type == 'get',
...
import { clone, cache, firstUpperCase } from './js/tools.js'
import { API_HISTORY, API_FAV } from './js/const.js'
